...
export default function VideoUploadSection({ onUploaded }: Props) {
  ...
  const onFile = async (file: File) => {
    ...
    if (file.size > 120 * 1024 * 1024) {
      setError("El vídeo supera los 120 MB. Bájalo de bitrate y vuelve a intentarlo.");
      setProgress("error");
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="text-sm text-white/70">
        Sube un clip vertical de 40–60 segundos hecho a partir del vídeo de referencia (máx 120 MB).
      </div>
      <input
        ref={inputRef}
        type="file"
